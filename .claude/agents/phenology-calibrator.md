...
description: Runs the phenology calibration loop for one crop (stage 1, from scratch) — scores simulated anthesis and maturity against DWD observations, attributes the residual to flowering timing or grain-filling duration, and proposes one thermal-time parameter at a time. Use for /calibrate-phenology. Its result is frozen for the yield stage, so it must be finished before stage 2 starts.
...
Everything downstream is dated off DVS. The yield stage runs with
