...
| Observations | `simplace/<crop>/data_observed/{phenology,yield}_<crop>.csv` — both at NUTS-3 DISTRICT resolution, not per site. `LAI_<crop>.csv` also exists but is on the Germany-wide BZE point set and does not overlap the Brandenburg sites; nothing scores it. |
| Site inventory + climate-grid join | `data/raw/point_to_nearest_grid.csv` (15 066 sites -> HYRAS cells) |
| CO2 forcing | `climate/co2/` (records) staged as each run dir's `data/co2/co2.csv` |
...
Read the actual files. Never answer from memory about a parameter's value, a
table's length, or a column's existence — the six crops differ, and getting that
wrong invalidates every downstream decision.

**Two facts about this model configuration that you must state when they bear on
the question, because they are easy to assume the other way:**

1. **CO2 is dynamic and scenario-specific.** `CO2Supply` reads
   `data/co2/co2.csv` and feeds `RadiationUseEfficiency.cCO`, which interpolates
   `crop.COTableFactor` against `crop.COTableCo2` to scale RUE and returns
   `RTMCO`, the transpiration-ratio modifier used by `LintulWaterStress`. So CO2
   acts on assimilation AND on water use. It was a fixed 380 ppm before this
   repository was restructured; the constant is still in the solution, commented.
2. **There is no vernalisation.** No crop.xml defines VBASE/VERSAT/VERNRT and no
   solution wires them in, so winter wheat and winter rapeseed develop on pure
   thermal time. A warm winter cannot delay them — which biases projected winter-
   crop anthesis early under warming scenarios.
