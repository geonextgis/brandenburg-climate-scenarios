...
# CO2 is staged, not symlinked: every solution reads data/co2/co2.csv and the
# source record varies by climate scenario. Calibration always runs on DWD
# observations, so it gets the observed record — co2_mm_historical.csv, which is
# the Mauna Loa series verbatim from 1958-03 plus a Law Dome reconstruction back
# to 1951-01. Using it rather than the raw observed file costs nothing for a
# post-1979 calibration window and removes the one way an earlier `year_start`
# could produce a NullPointerException on a month with no row.
CO2_SOURCE = "co2_mm_historical.csv"
...
def resolve_repo_root(cfg: dict) -> Path:
    """Repo root: explicit config > BRANDENBURG_SCENARIOS_ROOT env var > this file's location.
    ...
    """
    raw = cfg.get("repo_root") or os.environ.get("BRANDENBURG_SCENARIOS_ROOT") or "auto"
...
def build_run_dir(spec: RunSpec, rebuild: bool = False) -> dict:
    ...
    co2_src = spec.repo_root / "climate" / "co2" / CO2_SOURCE
...
def run_simplace(spec: RunSpec, iteration: int, log_path: Path | None = None) -> None:
    ...
    proc = subprocess.run(
        [sys.executable, str(spec.repo_root / "simplace" / "runners" / runner),
         str(spec.run_config)],
...
def _read_output_file(path: str) -> pd.DataFrame:
    """One per-location output CSV, with ``Location`` guaranteed to be present.

    The solution's daily header declares ``Location`` explicitly; the yearly one
    (``LintulYearly``) does not, so a yearly frame carries only ``projectid`` and
    ``simulationid`` to identify itself. The file name is the location either way
    — SIMPLACE writes ``${vLocationID}_<kind>.csv``, one file per location — which
    is how ``orchestration/consolidate_outputs.py`` recovers the PointID too. Fill
    it in from the stem rather than requiring every solution to be re-patched and
    every finished experiment to be re-run.
    """
    df = pd.read_csv(path, delimiter=";")
    if "Location" not in df.columns:
        df.insert(0, "Location", int(Path(path).stem.split("_")[0]))
    return df
...
def read_outputs(out_dir: Path, kind: str, max_workers: int = 32,
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as ex:
        frames = list(ex.map(_read_output_file, paths))
