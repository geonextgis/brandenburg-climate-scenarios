...
def phenology_diagnostics(df: pd.DataFrame, points: pd.DataFrame | None = None) -> dict:
    ...
    # `PointID` here holds the NUTS-3 code: the phenology observations are per
    # district, so objectives.phenology_process_result averages the simulated
    # sites into districts before joining. Group keys are therefore strings.
    per_district = df.groupby("PointID").apply(
        lambda g: float((g["flowering_doy_sim"] - g["flowering_doy"]).median()),
        include_groups=False)

    out = {
        "n_pairs": int(len(df)),
        "n_districts": int(df["PointID"].nunique()),
        "n_sites_averaged": int(df.attrs.get("n_sites", 0)),
        ...
        "residual_structure": {
            ...
            "worst_districts": [
                {"NUTS_ID": str(k), "flowering_bias": float(per_district[k])}
                for k in per_district.abs().nlargest(8).index
            ],
        },
        "by_year": by_year,
...
def load_points(run) -> pd.DataFrame | None:
    """Phenology unit -> latitude, for the photoperiod signature. None if unavailable.

    The phenology pairs are keyed by NUTS-3 district (see objectives.py), so the
    latitude that belongs beside a residual is the district's mean site latitude,
    not any single site's. The key column keeps the name ``PointID`` so the join
    in phenology_diagnostics() and phenology_plots() is unchanged.

    Note that Brandenburg spans about 2.2 degrees of latitude against Germany's
    7.5, so the photoperiod signature has correspondingly less leverage here —
    a flat slope is weak evidence rather than proof there is no signal.
    """
    path = run.repo_root / "data" / "raw" / "point_to_nearest_grid.csv"
    if not path.exists():
        return None
    pg = pd.read_csv(path, usecols=["PointID", "NUTS_ID", "Latitude", "Longitude"])
    return (pg.groupby("NUTS_ID", as_index=False)[["Latitude", "Longitude"]].mean()
            .rename(columns={"NUTS_ID": "PointID"}))
