...
```
  stage 1                          stage 2
  ┌───────────────────┐            ┌──────────────────────────────────┐
  │ phenology         │  handoff   │ growth = yield                   │
  │ from scratch      │ ─────────► │ phenology frozen                 │
  │ TSUM1/TSUM2/…     │            │ one crop.xml, one simulation     │
  └───────────────────┘            └──────────────────────────────────┘
```

**Why the growth stage is yield-only here, and what that costs.** The
Germany-wide project this layer was adapted from calibrated LAI and yield
*jointly*, and for a good reason: radiation use efficiency, light interception
and dry-matter partitioning set biomass _and_ leaf area, so calibrated in
sequence every accepted yield change silently rewrites the canopy — while
freezing the canopy to prevent that removes most of the yield levers with it.

That physics is unchanged. What is missing is the data. The inherited
`data_observed/LAI_<crop>.csv` files are on the Germany-wide BZE point set, which
has **zero overlap** with the Brandenburg 1 km sites, and `maize` and
`sugar_beet` have no LAI file at all. So there is one view, one component, and
**the canopy is unobserved**: a canopy parameter moved in stage 2 is measured
only through the yield it produces.

The consequence is a real risk, not a footnote. A yield error can be "fixed" by
an implausible canopy — excess leaf area compensating for too little RUE, or the
reverse — and the objective will improve while the model gets worse. The
`crop_reference` envelope (harvest index, above-ground biomass) in
`calibration.yaml` is the only guard, so the yield attribution has to be checked
against it every iteration and not only at the end.

Restoring the joint stage needs two files per crop: an `LAI_<crop>.csv` keyed on
Brandenburg PointIDs, and a matching `project_<crop>_LAI.csv`. With those, re-add
the `lai` view in `config.yaml` and its component in `calibration.yaml`; nothing
else in this layer changes.
...
## Stage 2 — growth (yield)

```bash
# 6. state, including the view and the objective scale
...
python optimization/calibrate.py run --crop winter_wheat --target growth \
    ...
    --hypothesis  "biomass is right and the harvest index is short — a partitioning error" \
    --reasoning   "ruled out RUE: raising it would push AGBiomass above the crop_reference ceiling" \
...
targets:
  growth:
    objective:
      name: yield_rmse_scaled
      components:
        yield: { weight: 1.0, scale: 0.50 } # mean of temporal + spatial RMSE (t/ha)
        # A second component (LAI) would be added here, with weights summing as
        # below, if LAI observations existed on these sites. See Stage 2 above.
```
...
- `config.yaml` is the `cluster:` block handed to
  `simplace/runners/simplace_runner_cluster.py` (shared, not per crop).
...
Both stages have a single view here, so nothing is currently mirrored — the
machinery is what would keep a re-added LAI view in step, and removing it would
mean the two components of a restored joint objective could describe different
parameter sets.
...
- `production` — `soil.csv`, `location.csv`, `fertilizer_<crop>.csv`. The only
  profile in this project; both stages use it.

The reference project had a second profile (`lai`) whose tables covered the
GLASS-LAI point set, which was distinct from the baseline set — that is why a
joint stage needed two runs rather than one. Re-adding an LAI view means
re-adding that profile alongside it.
...
| Two views drift apart                      | The primary view's `crop.xml` is mirrored into every other view before each run, after every rollback, and on every restore. (Single view per stage today, so nothing is mirrored yet.)         |
| A yield change quietly wrecks the canopy   | **Not guarded — the canopy is unobserved here.** The `crop_reference` envelope in `calibration.yaml` is reported in the yield attribution every iteration and is the only check. See Stage 2.   |
...
| `GrowthAgent`    | stage 2 — yield (the canopy is unobserved; see above)            |
