...
# ``flowering_doy`` / ``maturity_doy``.
#
# THE JOIN IS AT DISTRICT LEVEL. The DWD phenology observations available for
# Brandenburg are reported per NUTS-3 district and season, not per site, while
# the model runs on 15 066 1 km sites. Simulated dates are therefore averaged to
# (district, harvest_year) before the join. Attaching one district observation to
# each of its ~800 sites instead would replicate that single measurement ~800
# times inside the RMSE: the loss would be dominated by how many sites a district
# happens to contain, and its apparent sample size would be three orders of
# magnitude larger than the evidence behind it.
#
# The unit column is still called ``PointID`` — it holds the NUTS-3 code — so the
# diagnostics and plots that group by it work unchanged. What they report is
# per-district, and the metric labels say so.
#
...
PHENOLOGY_OBS_COLUMNS = ["NUTS_ID", "harvest_year", "flowering_doy", "maturity_doy"]
...
def phenology_process_result(spec: common.RunSpec) -> pd.DataFrame:
    ...
    # Sites -> districts, because that is the resolution of the observations.
    districts = (pd.read_csv(spec.project_csv, sep=";",
                             usecols=["vLocationID", "vNUTSID"])
                 .drop_duplicates()
                 .rename(columns={"vLocationID": "PointID", "vNUTSID": "NUTS_ID"}))
    n_sites = int(sim["PointID"].nunique())
    sim = sim.merge(districts, on="PointID", how="inner")
    sim = (sim.groupby(["NUTS_ID", "harvest_year"], as_index=False)
           [["flowering_doy_sim", "maturity_doy_sim"]].mean())

    obs = pd.read_csv(spec.crop_dir / "data_observed" / f"phenology_{spec.crop}.csv",
                      usecols=PHENOLOGY_OBS_COLUMNS)

    merged = sim.merge(obs, on=["NUTS_ID", "harvest_year"], how="inner")
    # Keep the unit column under the name the diagnostics group by.
    merged["PointID"] = merged["NUTS_ID"]
...
def phenology_loss_fn(df: pd.DataFrame) -> tuple[float, dict]:
    ...
    metrics = {
        ...
        "matched district-years": int(len(df)),
        "districts": int(df["PointID"].nunique()),
        "sites averaged into them": int(df.attrs.get("n_sites", 0)),
        "excluded (autumn bolting, site-years)": int(df.attrs.get("n_bolting", 0)),
    }
    return loss, metrics
...
def yield_process_result(spec: common.RunSpec) -> pd.DataFrame:
    ...
    points = (
        pd.read_csv(spec.project_csv, sep=";",
                    usecols=["vLocationID", "vNUTSID", "vSTATE_NAME"])
        .drop_duplicates()
        .rename(columns={"vLocationID": "location", "vNUTSID": "NUTS_ID",
