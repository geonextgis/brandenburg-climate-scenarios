...
def _():
    ...
    # Only ENABLED parameters are checked. A disabled one is deliberately absent
    # and carries a `disabled_reason` saying so (see the switch test below), so
    # requiring it to exist in some crop.xml would be checking the opposite of
    # what the switch means.
    declared = set()
    cfg = cc.load_calib_config(CONFIG)
    for target in TARGETS:
        for name, pc in (cfg["targets"][target].get("parameters") or {}).items():
            if pc.get("enabled") is not False:
                declared.add(name)
...
@test("canopy and yield parameters live in ONE stage, so neither can undo the other")
def _():
    # A parameter that moves biomass and leaf area (RUE, KDIF, partitioning) must
    # be calibratable in the same stage as yield. In this project only yield is
    # scored (there are no LAI observations on the Brandenburg sites), so the
    # canopy levers must still be in the space — they are how a yield error gets
    # fixed — but nothing measures what they do to leaf area. See the note on the
    # growth target in optimization/config.yaml.
    growth = cc.load_spec(CONFIG, "winter_wheat", "growth")
    ...
    assert set(growth.views) == {"yield"}, growth.views
...
def _():
    # Values chosen against THIS repo's winter_wheat SLA profile
    # ([0.011, 0.011, 0.011, 0.0135, 0.0125, 0.0115, 0.011, 0.012]) to sit inside
    # the bounds and inside the 50% step limit, so it is the shape rule under test
    # and not one of the cheaper guards.
    spec = cc.load_spec(CONFIG, "winter_wheat", "growth")
    # node 2: 0.011 -> 0.0069 (-37%). Node 3 then sits 1.96x above it, past 1.9.
    _, ids = proposal_check(spec, {"SLATableSLA": {"2": 0.0069}})
    assert "sla_profile_smooth" in ids, ids
    # node 7 (maturity): 0.012 -> 0.017 (+42%), above the 0.0135 profile maximum.
    _, ids = proposal_check(spec, {"SLATableSLA": {"7": 0.017}})
...
def _():
    # winter_wheat node 0 is leaves 0.40 / stems 0.60. Moving leaves alone to 0.54
    # (+35%, its upper bound) breaks the sum without tripping bounds or step size.
    spec = cc.load_spec(CONFIG, "winter_wheat", "growth")
    _, ids = proposal_check(spec, {"LeavesPartitioningTableFraction": {"0": 0.54}})
    assert "above_ground_partitioning_sums_to_one" in ids, ids
    # ...and passes when the stem fraction takes the difference.
    violations, _ = proposal_check(spec, {"LeavesPartitioningTableFraction": {"0": 0.54},
                                          "StemsPartitioningTableFraction": {"0": 0.46}})
    assert not violations, [v.message for v in violations]
...
def _():
    ...
    #
    # This repo's winter_wheat RUE table is [3.58, 3.09, 1.56, 0.93] — steep
    # enough that node 2 CANNOT be raised above node 1 without also leaving its
    # bounds and the 50% step limit. That is a property of the profile, not a gap
    # in the rule, so the violating case asserts membership rather than exclusivity
    # and the clean case is what proves the rule is not firing spuriously.
    spec = cc.load_spec(CONFIG, "winter_wheat", "growth")
    require_params(spec, "RUETableRUE")
    _, ids = proposal_check(spec, {"RUETableRUE": {"2": 4.2}})   # 1.56 -> 4.2, above 3.09
    assert "rue_declines_after_anthesis" in ids, ids
    # A legitimate decrease: 1.56 -> 1.2 (-23%), inside bounds and step limit.
    violations, ids = proposal_check(spec, {"RUETableRUE": {"2": 1.2}})
    assert "rue_declines_after_anthesis" not in ids, ids
    assert not violations, [v.message for v in violations]

    # winter_rapeseed storage-organ profile is [0, 0, 0, 0, 0.3, 0.6, 1, 1]:
    # 8 nodes, and likewise too steep to invert inside the step limit.
    spec = cc.load_spec(CONFIG, "winter_rapeseed", "growth")
    require_params(spec, "StorageOrgansPartitioningTableFraction")
    _, ids = proposal_check(spec, {"StorageOrgansPartitioningTableFraction": {"4": 0.7}})
...
@test("every view of a stage is driven from one crop.xml")
def _():
    # If a view kept its own parameter file, the components of the objective would
    # describe different crops. The growth stage has a single view here, so the
    # check is that the mirroring machinery still holds for whatever views exist —
    # it is what would keep a re-added LAI view in step.
    spec = cc.load_spec(CONFIG, "winter_wheat", "growth")
    assert spec.views, "the growth stage must have at least one view"
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        # The leading view IS spec.crop_xml, so only the trailing ones are mirrored.
        assert [str(p) for p in mirrored] == \
            [str(spec.runs[v].crop_xml) for v in spec.views[1:]]
        for view in spec.views:
            value = cc.read_param(cc.crop_root(spec.runs[view].crop_xml),
                                  spec.crop_name, "RGRLAI")
...
@test("vernalisation is offered only where the model actually consumes it")
def _():
    # Since 2026-09-07 the two winter crops vernalise: their crop.xml defines
    # VBASE/VERSAT/VERNRT at IDSL=2 and their solutions declare the three in the
    # `crop` resource and wire cVBASE/cVERSAT/cVERNRT into the LINTUL5 Phenology
    # component (orchestration/patch_crop_vernalisation.py). The four spring and
    # summer crops define none of them and drop out of the space on their own.
    #
    # The invariant under test is not "vernalisation exists" but "the parameter
    # space never offers a lever that writes to nothing" — which is exactly the
    # failure mode the wiring has: crop.xml alone changes nothing, silently.
    for crop in CROPS:
        spec = cc.load_spec(CONFIG, crop, "phenology")
        xml = spec.run.crop_dir / "data" / "crop" / "crop.xml"
        defined = cc.read_param(cc.crop_root(xml), spec.crop_name, "VERSAT") is not None
        solution = (spec.run.crop_dir / "solution" / "solution.sol.xml").read_text()
        wired = 'source="crop.VERSAT"' in solution
        names = set(spec.space.get("single_value_params") or {})
        if "VERSAT" in names:
            assert defined and wired, \
                f"{crop}: VERSAT is calibratable but the model does not consume it"
            row = next(r for r in cc.describe_space(spec) if r["parameter"] == "VERSAT")
            low, high = row["bounds"]
            assert row["movable"] and low <= row["value"] <= high, row
...
def _stub_agent(replies, *, valid=True, violations=()):
    ...
    status = {
        "crop": "winter_wheat", "target": "growth", "objective": "yield_rmse_scaled",
...
@test("the growth context names every component and its diagnostics")
def _():
    spec = cc.load_spec(CONFIG, "winter_wheat", "growth")
    agent, _ = _stub_agent([])
    context = agent.context(agent.status(), agent.history())
    ...
    weights = {c: spec.components[c]["weight"] for c in spec.components}
    for weight in weights.values():
        assert f"weight {weight}" in context, \
            f"the model must see how the components are weighted ({weights})"
    assert "attribution" in context, "the yield diagnostic block must reach the model"
...
def _():
    ...
    required = {
        # "canopy effect" and "reference ranges" are the two rules that replace the
        # joint objective's "predict both components": with LAI unobserved here,
        # the prompt has to make the agent state the unmeasured side effect and
        # check the agronomic envelope.
        "growth.md": ["Peak timing", "sum to one", "expected_effect", "FRTDM",
                      "canopy effect", "reference ranges"],
