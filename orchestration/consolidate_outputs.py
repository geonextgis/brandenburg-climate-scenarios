...
Each finished experiment leaves up to 15 066 tiny files behind — one per
Brandenburg site that reached harvest:
...
(~1.5 M inodes for the full 102-experiment matrix: 6 crops x 17 climates). This
collapses each experiment's `yearly/` directory into a single table:
...
Usage:
  ...
  python orchestration/consolidate_outputs.py --crop maize --climate DWD
...
def resolve_repo_root(cfg: dict) -> Path:
    """Repo root: explicit config > BRANDENBURG_SCENARIOS_ROOT > this file's location.
    ...
    is what makes BRANDENBURG_SCENARIOS_ROOT usable without editing experiments.yaml.
    """
    raw = cfg.get("repo_root")
    if raw is None or str(raw).strip().lower() in AUTO:
        raw = os.environ.get("BRANDENBURG_SCENARIOS_ROOT") or "auto"
...
class Experiment:
    ...
    def climate(self) -> str:
        # exp_id IS the climate id: this project has no soil-scenario dimension,
        # so there is nothing to split off the end.
        return self.exp_id


def discover(root: Path, cfg: dict, crops, climates) -> list[Experiment]:
...
def main(argv=None) -> int:
    ...
    exps = discover(root, cfg, set(a.crop), set(a.climate))
    if not exps:
...
def write_manifest(out_dir: Path, results: list[Result]) -> None:
    ...
    cols = ["crop", "exp_id", "climate", "n_location_files", "n_rows",
