...
WHY
---
The observed CO2 record (``climate/co2/co2_mm_observed.csv``) is the
...
The CO2 records live once, in ``climate/co2/``, and generate.py stages the one
matching each experiment's climate as that run dir's ``data/co2/co2.csv``. They
are a property of the scenario, not of the crop, so there is a single copy.

Usage:
  python orchestration/build_co2_historical.py            # (re)build the historical record
  python orchestration/build_co2_historical.py --check    # verify every record, write nothing
"""
...
SRC_NAME = "co2_mm_observed.csv"       # the Mauna Loa monthly record
...
def build(repo: Path, co2_dir: Path) -> pd.DataFrame:
    obs = pd.read_csv(co2_dir / SRC_NAME)
...
def main() -> int:
    ...
    repo = REPO_ROOT
    co2_dir = repo / cfg["paths"]["co2_dir"]
    problems: list[str] = []

    out_path = co2_dir / OUT_NAME
    df = build(repo, co2_dir)
    problems += verify(df, OUT_NAME)

    lo = f"{df.year.iloc[0]}-{df.month.iloc[0]:02d}"
    hi = f"{df.year.iloc[-1]}-{df.month.iloc[-1]:02d}"
    note = (f"{OUT_NAME:26s} {lo} .. {hi}  rows={len(df):,}  "
    ...
    # The historical record must reach back before the earliest window of any
    # climate that uses it — the historical period, not just the baseline.
    hist = cfg["periods"]["historical"]
    if df.year.min() > hist["start"]:
        problems.append(f"{OUT_NAME}: starts {df.year.min()}, "
                        f"historical runs start {hist['start']}")

    # Every SSP record must cover the whole future period with no holes.
    fut = cfg["periods"]["future"]
    for name in sorted(cfg["co2"]["by_ssp"].values()):
        f = pd.read_csv(co2_dir / name)
        ...
        print(f"  [checked]     {name:26s} {f.year.min()} .. {f.year.max()}  "
              f"rows={len(f):,}")
